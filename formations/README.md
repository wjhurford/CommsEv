...
vehicles, made by dragging them about the map until it looked right. Both are
legitimate, and they are stored differently for exactly that reason — a
function has parameters, a drawing has coordinates.

What stays parametric is the **size**. Each file records the spacing it was
drawn at (the closest gap between any two of its vehicles), so asking for the
shape at another spacing scales every offset by the ratio. The shape is
preserved; only how big it is remains a decision made later.
...
only thing that creates a file, so this folder holds exactly the shapes you
