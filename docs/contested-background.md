...
The **scene owns the background**: every condition that exists before any
hostile actor does. The test for whether something belongs here is a single
question: whether the condition would hold in the world if no adversary
existed. If yes, it is background and the scene declares its baseline. If no —
a jammer, a spoofer, a deliberate emitter — it is an **agent** (or an attack),
and it belongs to a fleet or to the Contested tab, acting on this baseline.

That split is what keeps the physics consistent: `rf_link()` computes SINR
with a jammer as *just another term in the noise denominator*. The
denominator's resting value is the scene's; the extra terms are attributable to
a specific emitter. Results then measure attacks as departures from a declared
baseline rather than from an assumption nobody wrote down.
...
cannot belong to any one radio or agent. It is currently declared with empty
...
is part of the world. This is deliberately binary today; the next refinement
(needed before GNSS *spoofing* can mean anything in the Contested tab) is a
received-quality figure — nominal C/N0 per constellation — because real-world
jamming presents as C/N0 depression long before outright denial, which is
exactly what the AIS/maritime GNSS RFI monitoring paper in the project library
detects fleet-wide. A spoofing attack then has a baseline to be measured
against.

**`wind`** — model (`none` indoors). Why here: the air is part of the world.
It is irrelevant to the ground cars and lidar, but once quadcopters fly, wind
...
outdoors will declare a real value.
...
- **Per-band conditions** — the background is currently one number per
  ...
  alternate / contingency / emergency network) is only measurable if the scene
  can make 900 MHz behave differently from 5.8 GHz.
- **Atmospheric attenuation** — considered and deliberately excluded for now:
  below ~10 GHz over lab-to-field ranges it is negligible relative to the other
  terms. It earns a place only when the extreme-bandwidth (mmWave+) regimes
  from the jamming survey in the project library are modelled; a placeholder
  would otherwise invite false precision.
...
## What must not enter the background
