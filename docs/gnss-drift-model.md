...
dead-reckons and DRIFTS. Built emergent, not hardcoded, and every number is a
declared free parameter with a source. See docs/jamming-effects-research.md
for the background and docs/jamming-model-justification.md for the honesty
line (trust trends, not exact metres).

## Two poses per agent

Each agent has a TRUE pose (the physics, in `poses`) and a BELIEF (`agent
["belief"]`) — where it thinks it is. `drift = belief - truth`, reported per
frame as `position_error_m`.

## The rule

Each tick, an agent has a position fix if EITHER:
- GNSS is available (the scene provides it) AND not denied by a GNSS-band
  jammer, OR
- it carries an RF-immune aiding sensor (lidar / camera) — it can localise
...
%-of-distance behaviour the literature reports — and it emerges, it is not a
scripted wander. **The controller steers from the belief** (a vehicle has no
other position), so a drifted car aims from a wrong origin and its true path
bends off-target: it "arrives" at its waypoint in its own estimate while
physically ending up `position_error_m` away, and doesn't know it.
...
Scene `open_field` (GNSS available), fleet `3_roboracer_no_lidar` (no aiding
sensor), plus `red_jammer`. Retune the jammer to the GNSS band:
...
dead reckoning, est. position error X m". `red halt` (or `JAM jam1 band 2400`)
and the fix returns, the ghosts snap back.

Swap the fleet for `3_roboracer` (has lidar) and the same GNSS jamming does
nothing — the lidar localises the car without GNSS. That contrast is the
GNSS-denied-navigation result, and the reason sensors matter.
...
- Lidar aiding is modelled as "has a lidar => localises => no drift", a scalar
  ...
  needs map features (a lidar in a featureless field would not actually help).
